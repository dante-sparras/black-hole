...
**Derived / locked:** **ṁ** (scenario/preset expert readout — not free), ℓ̃ ≈ √(r_in/M), **ISCO** (HUD reference only — r_in is free), SANE/MAD from β₀; structure defaults; tilt node 0. Disk always co-rotates (L ‖ J); use signed a★ for spin direction.
